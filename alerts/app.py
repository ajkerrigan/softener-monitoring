...
app = Flask(__name__)
app.config['API_KEY'] = Path('/run/secrets/sendgrid_api_key').read_text()
app.config['MAIL_FROM'] = Email(Path('/run/config/mail_from').read_text())
app.config['MAIL_TO'] = To(Path('/run/config/mail_to').read_text())
